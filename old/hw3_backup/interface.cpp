...
/*int main(int argc, char *argv[]) {
    ...
    int err1 = openBMP(&PARAMS, &P1, fileName);
    int err2 = initGeometries(&PARAMS, &G1);
    ...
    case 1: // add the numbers on the CPU
        ...
        start = clock();
        runIt(&P1, &G1, &PARAMS); // always run on GPU because CPU note supported
        end = clock();
        break;
...
int runIt(GPU_Palette* P1, GPU_Geometries *G1, AParams* PARAMS) {
    ...
    while (1) {
        if (PARAMS->verbosity)
            printf("Iteration: %d \n", i++);
        drawGeometries(*P1, *G1);
        updateGeometries(*P1, *G1);
...
int initGeometries(AParams* p, GPU_Geometries* G1) {
    ...
    int NUM_GEOMETRIES = 2;
    int MEM_SIZE = NUM_GEOMETRIES * sizeof(Geometry);

    (*G1) = initGPUGeometries(NUM_GEOMETRIES, MEM_SIZE);


    Geometry *geo_list = new Geometry [NUM_GEOMETRIES];
    // TODO: Hard coded to one now
    geo_list[0].type = CIRCLE;
    Circle c = {.center = newPoint(60, 60),
                .radius = 50,
                .fill = true
               };
    geo_list[0].shape.circle = c;
    geo_list[0].color = newColor(0.99, 0, 0);
    geo_list[0].thickness = 3;
    Properties prop = {.momentumX = 10, .momentumY = 1};
    geo_list[0].properties = prop;


    geo_list[1].type = CIRCLE;
    Circle c2 = {.center = newPoint(200, 60),
                .radius = 20,
                .fill = false
               };
    geo_list[1].shape.circle = c2;
    geo_list[1].color = newColor(0, .99, .99);
    geo_list[1].thickness = 3;
    Properties prop1 = {.momentumX = -2, .momentumY = 30};
    geo_list[1].properties = prop1;

    cudaMemcpy(G1->geometries, geo_list, MEM_SIZE, cH2D);
    G1 -> size = NUM_GEOMETRIES;
...
int openBMP(AParams* PARAMS, GPU_Palette* P1, char* fileName) {
    ...
    cudaMemcpy(P1->gray_background, graymap, P1->gSize, cH2D);
    cudaMemcpy(P1->red_background, redmap, P1->gSize, cH2D);
    cudaMemcpy(P1->green_background, greenmap, P1->gSize, cH2D);
    cudaMemcpy(P1->blue_background, bluemap, P1->gSize, cH2D);
    // free CPU memory
    free(graymap);
    free(redmap);
    free(greenmap);
