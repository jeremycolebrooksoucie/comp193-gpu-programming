...
void CPUAnimBitmap::drawPalette() {
  ...
  unsigned char *cp_bitmap = dev_bitmap;
  unsigned char *cp_pixels = pixels;

  for (int y = 0; y < print_height; y++ )
  {
      cudaMemcpy(cp_pixels, cp_bitmap, print_width * 4, cudaMemcpyDeviceToHost);
      cp_pixels += 4 * print_width;
      cp_bitmap += 4 * gWidth;
  }
...
/*************************************************************************/
/*************************************************************************/
CPUAnimBitmap::CPUAnimBitmap(AParams *p, GPU_Palette* d) {//void* d) {
  gWidth       = p -> width;
  gHeight      = p -> height;
  print_width  = p -> print_width;
  print_height = p -> print_height;

  pixels = new unsigned char[print_image_size()];

  cudaMalloc((void **) &dev_bitmap, gImage_size());
...
void CPUAnimBitmap::mouse_func(int button, int state, int mx, int my) {
  ...
  // broke this as it was seg faulting and functinality isn't needed
}
...
void CPUAnimBitmap::Draw(void) {
  ...
  glDrawPixels(bitmap->print_width, bitmap->print_height, GL_RGBA, GL_UNSIGNED_BYTE,
               bitmap->pixels);
  glutSwapBuffers();
}
...
void CPUAnimBitmap::initAnimation() {
  ...
  *bitmap = this;
  ...
  glutInitWindowSize(params->print_width, params->print_height);
